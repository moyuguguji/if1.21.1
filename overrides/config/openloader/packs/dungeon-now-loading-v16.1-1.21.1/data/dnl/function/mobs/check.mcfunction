...
execute as @e[type=area_effect_cloud,tag=dnl.hunter_name] unless entity @e[type=skeleton,tag=dnl.hunters_ghost,distance=..2] run kill @s
...
# 奖励改由进度 dnl:items/broken_bow/killed_hunter_ghost 触发(击杀时手持 Broken Bow), 不再依赖掉落信标
...
execute as @e[type=item,nbt={Item:{components:{"minecraft:custom_data":{station_chief_killed:1b}}}}] run function dnl:mobs/railway_outpost/station_chief_killed
...
execute as @e[type=item,nbt={Item:{components:{"minecraft:custom_data":{frost_watcher_killed:1b}}}}] run function dnl:mobs/watcher_camp/frost_watcher_killed
